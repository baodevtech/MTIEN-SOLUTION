...
import { Calendar, ArrowRight, Search, ChevronRight, Hash, Loader2, Filter } from 'lucide-react';
...
export default function BlogListClient({
  ...
  // Fetch filtered posts with debounce & race condition handling
  useEffect(() => {
    let isMounted = true;
          ...
          {isFiltering ? (
            ...
            <div className="space-y-10 md:space-y-12">
              {posts.map((post, index) => {
                ...
                return (
                  <article
                    key={post.id}
                    className="flex flex-col group border-b border-gray-100 pb-10 md:pb-12 last:border-0 last:pb-0"
                  >
                    <Link
                      href={`/blog/${post.slug}`}
                      className="block overflow-hidden rounded-2xl aspect-[16/9] mb-5 md:mb-6 shadow-sm relative bg-slate-100"
                    >
                      {imageUrl ? (
                        <Image
                          src={imageUrl}
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-[10px] md:text-xs font-bold uppercase text-slate-400 mb-3">
                      {post.category && (
                        <span className="text-amber-600 bg-amber-50 px-2.5 py-1 rounded-md">
                          {post.category}
                        </span>
                      )}
                    ...
                    {post.excerpt && (
                      <p className="text-slate-500 text-sm md:text-base leading-relaxed mb-5 line-clamp-3">
                        {post.excerpt}
                      </p>
                    )}
                    ...
                    <div className="flex items-center justify-between mt-auto">
                      ...
                      <Link
                        href={`/blog/${post.slug}`}
                        className="flex items-center gap-1 text-xs md:text-sm font-bold text-amber-600 hover:text-amber-800"
                      >
            ...
            <div className="flex flex-col items-center justify-center py-24 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
              ...
              <p className="text-sm text-slate-500 mb-6 max-w-sm text-center">
                Rất tiếc, không có bài viết nào khớp với từ khóa của bạn.
              </p>
              <button
                onClick={() => {
                  setFilter('All');
                ...
                className="px-6 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-bold text-amber-600 shadow-sm hover:border-amber-200"
              >
                Xóa bộ lọc
              </button>
            </div>
        ...
        <aside className="lg:w-1/3 space-y-10">
          <div className="hidden lg:block space-y-10">
            ...
            <nav aria-label="Danh mục">
              <h3 className="font-bold text-slate-900 text-lg mb-5 border-b border-gray-100 pb-3">
                Danh mục
              </h3>
                  ...
                  {allTags.map((tag) => {
                    ...
                    return (
                      <button
                        key={`desktop-tag-${tag}`}
                        onClick={() => setSelectedTag(isActive ? '' : tag)}
                        className={`flex items-center gap-1 px-3.5 py-2 border rounded-lg text-xs font-bold transition-all ${
                          isActive
                            ? 'bg-amber-500 border-amber-500 text-white'
                            : 'hover:text-amber-600'
                        }`}
                      >
                        <Hash size={12} className={isActive ? 'text-white/70' : 'text-slate-400'} />{' '}
                        {tag}
                      </button>
                    );
                  })}
          ...
          {/* Bài viết nổi bật — visible on both mobile & desktop */}
          <div>
            <h3 className="font-bold text-slate-900 text-lg mb-5 border-b border-gray-100 pb-3">
              Bài viết nổi bật
            </h3>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      {post.category && (
                        <span className="text-[10px] font-bold text-amber-600 mb-1 block uppercase">
                          {post.category}
                        </span>
                      )}
                      <h4 className="text-sm font-bold text-slate-900 line-clamp-2 group-hover:text-amber-600 transition-colors">
                        {post.title}
                      </h4>
                    </div>
                  </Link>
