...
import { Calendar, ArrowRight, Search, ChevronRight, Hash, Loader2, Tag } from 'lucide-react';
...
export default function BlogListClient({ 
  ...
  // Khởi tạo state từ URL Params (tốt cho SEO & Chia sẻ link)
  ...
  // Cập nhật URL khi bộ lọc thay đổi (giúp chuẩn SEO, có thể copy link URL)
  useEffect(() => {
  ...
  }, [filter, search, selectedTag, router]);

  // EFFECT: Gọi API khi Filter, Search hoặc Tag thay đổi với xử lý Race Condition
  useEffect(() => {
    let isMounted = true; // Cờ kiểm tra component còn mount hay không
    ...
    return () => {
        isMounted = false; // Cleanup function: Tránh cập nhật state của request cũ
  ...
  return (
    <div>
        ...
        <div className="lg:hidden mb-6 flex flex-col gap-3">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 snap-x snap-mandatory [&::-webkit-scrollbar]:hidden">
                <button 
                    ...
                    className={`snap-start whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${
                        filter === 'All' ? 'bg-slate-900 border-slate-900 text-white shadow-sm' : 'bg-white border-slate-200 text-slate-600'
                    }`}
                >
                    Tất cả
                </button>
            ...
            {allTags.length > 0 && (
                <div className="flex gap-1.5 overflow-x-auto pb-1 [&::-webkit-scrollbar]:hidden items-center">
                    <div className="text-[10px] font-bold uppercase text-slate-400 shrink-0 flex items-center gap-1">
                        <Tag size={12}/> Tags
                    </div>
                    {allTags.map(tag => {
                ...
                {isFiltering ? (
                    ...
                    <div className="space-y-10 md:space-y-12">
                        {posts.map((post, index) => {
                            ...
                            const dateStr = post.publishedAt ? new Date(post.publishedAt).toLocaleDateString('vi-VN', { year: 'numeric', month: 'short', day: 'numeric' }) : '';

                            return (
                                <article key={post.id} className="flex flex-col group border-b border-gray-100 pb-10 md:pb-12 last:border-0 last:pb-0">
                                  <Link href={`/blog/${post.slug}`} className="block overflow-hidden rounded-2xl aspect-[16/9] mb-5 md:mb-6 shadow-sm relative bg-slate-100">
                                        {imageUrl ? (
                                            <Image 
                                                src={imageUrl} 
                                                alt={post.title} 
                                    ...
                                    <div className="flex items-center justify-between mt-auto">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 md:w-10 md:h-10 rounded-full overflow-hidden bg-slate-200 relative">
                                            </div>
                ...
                <div className="hidden lg:block space-y-10">
                    ...
                    <nav aria-label="Danh mục">
                        ...
                        <ul className="space-y-1.5">
                            <li key="desktop-cat-all">
                                <button 
                                    onClick={() => setFilter('All')}
                                    className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all ${filter === 'All' ? 'bg-amber-50 text-amber-700 font-bold' : 'hover:bg-slate-50 font-medium'}`}
                                >
                                    <span>Tất cả bài viết</span><ChevronRight size={14} />
                                </button>
                            </li>
                            {categories.map(cat => (
