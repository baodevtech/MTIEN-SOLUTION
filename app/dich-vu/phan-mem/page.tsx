...
import { CheckCircle2, XCircle, Plus, Minus, ArrowRight, Layers, Globe, Smartphone, Users, ShieldCheck, Zap, Sparkles, Cloud, Check, Cpu, BarChart3 } from 'lucide-react';
import { Manrope, Noto_Sans } from 'next/font/google';
import { Inter } from 'next/font/google';

// Chỉ sử dụng Inter cho toàn bộ trang để tạo sự đồng nhất tuyệt đối
const inter = Inter({
  subsets: ['latin'],
...
// --- DỮ LIỆU HỆ SINH THÁI (RADIAL ECOSYSTEM) ---
// Đừng quên import thêm: Cpu, BarChart3 từ 'lucide-react'
const ecosystemData = [
  {
...
export default function ServicesPage() {
  ...
  // States cho Section 3 (Radial Ecosystem)
  ...
  return (
    <main className={`${notoSans.className} bg-white text-[#616161]`}>
      ...
      <section className="relative bg-[#061153] pt-[150px] pb-[130px] overflow-hidden rounded-b-[40px] md:mx-5 mt-5 shadow-2xl shadow-blue-900/10 border border-blue-950/20">
        ...
        <div className="container mx-auto px-6 relative z-30 flex flex-col items-start gap-10">
          <div className="w-full">
            <motion.h1 initial="hidden" animate="visible" variants={titleVariants} className={`${manrope.className} text-4xl md:text-[62px] font-extrabold text-white mb-8 leading-[1.1] tracking-tighter drop-shadow-[0_2px_15px_rgba(16,83,243,0.3)]`}>
      ...
      {/* 3. RADIAL ECOSYSTEM - PURE TYPOGRAPHY STYLE */}
      <section className={`${inter.className} py-[120px] lg:py-[180px] bg-white relative overflow-hidden`}>

        {/* Background Decor - Mesh Gradient siêu nhẹ */}
        <div className="absolute inset-0 pointer-events-none opacity-30">
          <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-50 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/4"></div>
        </div>

        <div className="container mx-auto px-6 max-w-[1300px] relative z-10">

          <div className="flex flex-col lg:flex-row items-center gap-[80px] lg:gap-[40px]">

            {/* LEFT: THE KINETIC ORBIT (Tối giản & Sang trọng) */}
            <div className="w-full lg:w-[45%] relative flex items-center justify-center aspect-square max-w-[480px]"
              onMouseEnter={() => setIsHovered(true)}
              onMouseLeave={() => setIsHovered(false)}>

              {/* Lõi trung tâm - Minimalist Nucleus */}
              <div className="relative z-20 w-[140px] h-[140px] rounded-full bg-white shadow-[0_30px_60px_rgba(0,0,0,0.06)] border border-slate-50 flex items-center justify-center">
                <div className="w-[80px] h-[80px] rounded-full bg-[#1d1d1f] flex items-center justify-center text-white">
                  <Zap size={30} fill="currentColor" />
                </div>
              </div>

              {/* Các Node dịch vụ - Floating Nodes */}
              {ecosystemData.map((node, i) => {
                const angle = i * (360 / ecosystemData.length);
                ...
                return (
                  <button
                    ...
                    className="absolute top-1/2 left-1/2 z-30 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]"
                    style={{
                  ...
                  >
                    <div className="relative flex flex-col items-center group">
                      <div className={`w-[70px] h-[70px] rounded-full flex items-center justify-center transition-all duration-500 border
                      ${isActive
                          ? 'bg-white shadow-[0_20px_40px_rgba(0,0,0,0.1)] border-slate-100 scale-125'
                          : 'bg-transparent border-transparent grayscale opacity-25 hover:opacity-100 hover:scale-110 hover:grayscale-0'
                        }`}
                        style={{ color: isActive ? node.hex : '#86868b' }}>
                        {React.cloneElement(node.icon as React.ReactElement, { size: 24 })}
                      </div>
                    </div>
                  </button>
                );
            ...
            {/* RIGHT: THE STUDIO CANVAS (Bố cục hiện đại, phóng khoáng) */}
            <div className="w-full lg:w-[55%] min-h-[600px] flex flex-col justify-center">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-12 border-b border-slate-100 pb-6">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-600">
                        {ecosystemData[activeService].label}
                      </span>
                    </div>

                    <div className="flex gap-4">
                      {ecosystemData[activeService].techs.slice(0, 3).map((t, i) => (
                  ...
                  {/* 2. MAIN CONTENT: Headline & Description */}
                  <div className="max-w-[600px] mb-12">
                    <h3 className="text-[44px] md:text-[62px] font-semibold text-[#1d1d1f] leading-[1.05] tracking-[-0.03em] mb-8">
                      {ecosystemData[activeService].title}
                    </h3>

                    <div className="flex gap-6">
                      {/* Thanh tiến trình dọc tinh tế */}
                      <div className="w-[2px] bg-slate-100 rounded-full overflow-hidden shrink-0">
                        <motion.div
                          key={activeService}
                          initial={{ height: "0%" }}
                          animate={{ height: isHovered ? "0%" : "100%" }}
                          transition={{ duration: AUTO_PLAY_DURATION / 1000, ease: "linear" }}
                          className="w-full bg-[#1d1d1f]"
                        />
                      </div>

                      <p className="text-[18px] md:text-[21px] text-slate-500 font-normal leading-[1.6] max-w-[480px]">
                        {ecosystemData[activeService].desc}
                      </p>
                    </div>
                  </div>

                  {/* 3. FEATURES GRID: Tối giản, không khung (Pills) */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 mb-12">
                    {ecosystemData[activeService].features.slice(0, 4).map((feature, i) => (
                      <div key={i} className="group cursor-default">
                        <div className="text-[15px] font-semibold text-[#1d1d1f] mb-2 flex items-center gap-2">
                          <div className="w-1 h-1 rounded-full bg-blue-600"></div>
                          {feature}
                        </div>
                        <div className="h-[1px] w-0 group-hover:w-full bg-slate-200 transition-all duration-500"></div>
                      </div>
                    ))}
                  </div>

                  {/* 4. ACTION: Premium Button */}
                  <div className="pt-4">
                    <a
                      href="#contact"
                      className="inline-flex items-center gap-6 group"
                    >
                      <div className="w-14 h-14 rounded-full bg-[#1d1d1f] flex items-center justify-center text-white transition-transform duration-500 group-hover:scale-110 group-hover:bg-blue-600">
                        <ArrowRight size={22} className="group-hover:translate-x-1 transition-transform" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[15px] font-semibold text-[#1d1d1f]">Khám phá chi tiết</span>
                        <span className="text-[12px] text-slate-400 font-medium tracking-wide">Nhận báo giá và tư vấn 24/7</span>
                      </div>
                    </a>
                  </div>
        ...
        <div className="bg-[#09155C] pt-[140px] pb-[120px] rounded-b-[40px] md:mx-5 relative z-0 overflow-hidden shadow-xl">
          ...
          <div className="container mx-auto px-6 relative z-10">
            <div className="text-center mb-[80px] -mt-[25px]">
              <h2 className={`${manrope.className} text-5xl md:text-7xl font-black text-white/5 tracking-[0.05em] uppercase leading-none absolute left-1/2 -translate-x-1/2 -top-4 w-full`}>QUY TRÌNH CHUẨN</h2>
              <h2 className={`${manrope.className} text-4xl md:text-5xl font-extrabold text-white tracking-tight relative z-10`}>Cách Chúng Tôi Làm Việc</h2>
            </div>
      ...
      <section className="py-[120px] bg-[#F4F7FF]">
        <div className="container mx-auto px-6">
          <div className="text-center mb-[80px]">
            ...
            <h2 className={`${manrope.className} text-3xl md:text-5xl font-extrabold text-[#061153] leading-tight tracking-tight`}>Minh Bạch Chi Phí <br /> Phù Hợp Mọi Quy Mô</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-[30px] items-end max-w-7xl mx-auto">
            <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" custom={0} viewport={{ once: true }} className="bg-white rounded-[30px] p-[40px] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-slate-100 hover:border-blue-100 flex flex-col h-full hover:-translate-y-2 transition-all duration-300">
              ...
              <div className="mb-4 flex items-baseline gap-2"><h2 className={`${manrope.className} text-4xl font-extrabold text-[#061153] leading-none tracking-tight`}>Web Cơ Bản</h2></div>
            ...
            <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" custom={0.2} viewport={{ once: true }} className="bg-white rounded-[30px] p-[50px] shadow-[0_20px_50px_rgba(16,83,243,0.15)] relative border-2 border-[#1053F3] md:-translate-y-6 flex flex-col h-full z-10 overflow-hidden group">
              ...
              <div className="mb-4 flex items-baseline gap-2 relative z-10"><h2 className={`${manrope.className} text-4xl font-extrabold text-[#061153] leading-none tracking-tight`}>Web + App</h2></div>
            ...
            <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" custom={0.4} viewport={{ once: true }} className="bg-white rounded-[30px] p-[40px] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-slate-100 hover:border-blue-100 flex flex-col h-full hover:-translate-y-2 transition-all duration-300">
              ...
              <div className="mb-4 flex items-baseline gap-2"><h2 className={`${manrope.className} text-4xl font-extrabold text-[#061153] leading-none tracking-tight`}>Custom ERP</h2></div>
        ...
        <div className="container mx-auto px-6 relative z-10 max-w-7xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-[60px] items-start">
            <motion.div variants={fadeInLeft} initial="hidden" whileInView="visible" viewport={{ once: true }} className="lg:col-span-5 sticky top-[100px]">
              ...
              <h2 className={`${manrope.className} text-3xl md:text-5xl font-extrabold text-[#061153] leading-tight tracking-tight mb-[24px]`}>Những Câu Hỏi <br /> Thường Gặp</h2>
              ...
              {faqs.map((faq, index) => (
                <div key={index} className={`border rounded-[24px] overflow-hidden transition-all duration-300 ${activeFaq === index ? 'border-[#1053F3]/30 shadow-[0_10px_30px_rgba(16,83,243,0.08)] bg-gradient-to-br from-blue-50/50 to-white' : 'border-slate-200 bg-white hover:border-[#1053F3]/30 hover:bg-blue-50/20'}`}>
                  <button onClick={() => setActiveFaq(activeFaq === index ? null : index)} className="w-full text-left px-8 py-6 flex items-center justify-between focus:outline-none">
                    <h4 className={`${manrope.className} text-lg font-extrabold pr-6 transition-colors duration-300 leading-snug tracking-tight ${activeFaq === index ? 'text-[#1053F3]' : 'text-[#061153]'}`}>{faq.question}</h4>
        ...
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-[80px] items-center">
            <motion.div variants={fadeInLeft} initial="hidden" whileInView="visible" viewport={{ once: true }}>
              ...
              <h2 className={`${manrope.className} text-4xl md:text-5xl font-extrabold text-[#061153] mb-[24px] leading-[1.2] tracking-tight`}>Hãy Để Chúng Tôi <br /> Hiện Thực Hóa Ý Tưởng </h2>
