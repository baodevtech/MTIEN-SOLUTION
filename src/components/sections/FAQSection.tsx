...
export default function FAQSection() {
  ...
  return (
    <section className="py-16 md:py-32 bg-[#F8FAFC] relative overflow-hidden" aria-label="Câu hỏi thường gặp">
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-20 items-start">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-28 flex flex-col">
            ...
            <h2 className="text-[24px] md:text-[44px] font-black text-slate-900 leading-[1.15] tracking-tight mb-3 md:mb-4">
              Giải đáp nhanh <br /><span className="text-[#0066FF]">mọi thắc mắc</span>
            </h2>
            ...
            {/* Helpdesk Card — compact on mobile */}
            <div className="bg-white rounded-xl md:rounded-[24px] p-5 md:p-8 shadow-sm md:shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-slate-100">
          ...
          <div className="lg:col-span-7 pt-2 lg:pt-0">
            <div className="space-y-3 md:space-y-4" role="region" aria-label="Danh sách câu hỏi thường gặp">
              {faqs.map((faq, idx) => (
                <div key={idx} className={`group bg-white rounded-xl md:rounded-[20px] transition-all duration-300 border ${activeFaq === idx ? 'border-[#0066FF] shadow-md md:shadow-[0_10px_40px_rgba(0,102,255,0.08)] relative z-10' : 'border-slate-200 shadow-sm'}`}>
                  <button onClick={() => setActiveFaq(activeFaq === idx ? null : idx)} className="w-full px-5 py-4 md:px-8 md:py-6 text-left flex justify-between items-center focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded-xl md:rounded-[20px]" aria-expanded={activeFaq === idx} aria-controls={`faq-answer-${idx}`}>
                    <span className={`font-bold text-[14px] md:text-[18px] pr-2 md:pr-6 transition-colors ${activeFaq === idx ? 'text-[#0066FF]' : 'text-slate-900'}`}>
                      {faq.question}
                    </span>
                    <div className={`shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all ${activeFaq === idx ? 'bg-[#0066FF] text-white' : 'bg-slate-50 text-slate-400'}`} aria-hidden="true">
                      {activeFaq === idx ? <Minus size={14} strokeWidth={2.5} /> : <Plus size={14} strokeWidth={2.5} />}
                    </div>
                  </button>
                  {reduced ? (
                    activeFaq === idx && (
                      <div id={`faq-answer-${idx}`} role="region">
                        <div className="px-5 md:px-8 pb-5 md:pb-8 pt-1">
