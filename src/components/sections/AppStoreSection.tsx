...
export default function AppStoreRedesign() {
  ...
  return (
    <section className="bg-[#FBFBFC] py-12 md:py-16 relative z-20 border-y border-slate-200/60 overflow-hidden font-sans lg:h-[90vh] flex items-center" aria-label="Hệ sinh thái ứng dụng">
      ...
      <div className="max-w-[1260px] mx-auto px-6 relative z-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-6 flex flex-col order-2 lg:order-1">
            <header className="mb-8">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-white shadow-sm border border-slate-100 rounded-full text-blue-600 text-[11px] font-bold mb-4 uppercase tracking-widest">
                <Cpu size={14} /> <span>Hệ sinh thái MTIEN SOLUTION</span>
              </div>
              <h2 className="text-4xl md:text-5xl font-bold text-slate-900 tracking-tighter leading-[1.1] mb-4">
                Mở rộng sức mạnh. <br/>
            ...
            <div className="grid grid-cols-4 gap-3 auto-rows-[90px] md:auto-rows-[105px]">
              {apps.map((app, idx) => (
                <motion.div
                  key={idx}
                  whileHover={{ y: -4, scale: 0.99 }}
                  className={`
                    relative rounded-[1.8rem] p-4 flex flex-col overflow-hidden group border border-white bg-white/70 backdrop-blur-xl transition-all shadow-[0_4px_20px_rgba(0,0,0,0.02)]
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${app.color} flex items-center justify-center text-white shadow-lg shadow-blue-500/10`}>
                      <app.icon size={18} strokeWidth={2.5} />
                    </div>
                    {app.tag && <span className="text-[9px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-tighter">{app.tag}</span>}
                  </div>
                  
                  <div className="mt-auto">
                    <h4 className="font-bold text-slate-900 text-[13px] tracking-tight">{app.name}</h4>
                    {app.desc && <p className="text-[11px] text-slate-400 mt-1 line-clamp-1 font-medium">{app.desc}</p>}
                  </div>
              ...
              <div className="col-span-2 row-span-1 bg-slate-900 rounded-[1.8rem] p-5 flex items-center justify-between group cursor-pointer hover:bg-blue-600 transition-all shadow-xl shadow-slate-900/10">
                 <div className="flex flex-col">
                    <span className="text-white font-bold text-sm tracking-tight">Khám phá 100+</span>
                    <span className="text-white/50 text-[10px] font-medium">Tiện ích tích hợp sẵn</span>
                 </div>
          ...
          <div className="lg:col-span-6 relative order-1 lg:order-2 h-[400px] lg:h-[500px] flex items-center">
            ...
            {!reduced && (
              <>
                <motion.div 
                  ...
                  className="absolute top-10 right-0 z-30 bg-white/90 backdrop-blur-xl p-3.5 rounded-2xl shadow-2xl border border-white flex items-center gap-3 ring-1 ring-black/5"
                >
                ...
                <motion.div 
                  ...
                  className="absolute bottom-10 -left-8 z-30 bg-white/95 backdrop-blur-xl p-4 rounded-[1.8rem] shadow-2xl border border-white flex items-center gap-4 min-w-[180px] ring-1 ring-black/5"
                >
            ...
            <div className="relative w-full aspect-square md:aspect-[4/3] bg-[#E8E8ED] rounded-[3.5rem] p-2.5 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.12)] border border-slate-300 ring-1 ring-white/50 overflow-hidden">
        ...
        <div className="mt-12 pt-8 border-t border-slate-200/60 flex flex-wrap justify-between items-center gap-6">
