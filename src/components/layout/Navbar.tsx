...
const mainLinks = [
  ...
  { href: '/shop', label: 'Cửa hàng', icon: Store, hasDropdown: false, isHot: true },
  { href: '#', label: 'Đối tác', icon: Users, hasDropdown: false, isHot: false },
  { href: '#', label: 'Bảng giá', icon: CreditCard, hasDropdown: false, isHot: false },
  { href: '#', label: 'Tin tức', icon: Newspaper, hasDropdown: false, isHot: false },
];
          ...
          <div className="bg-gradient-to-r from-[#001440] via-[#002f90] to-[#001440] text-blue-50 text-[12px] h-full flex justify-center items-center px-4 relative">
            <div className="flex items-center gap-4 max-w-7xl mx-auto w-full justify-between lg:justify-center">
              <div className="hidden lg:flex items-center gap-5 text-blue-100/90 font-medium">
                <a href="tel:18006750" className="flex items-center gap-1.5 hover:text-white transition-colors group">
                  <Phone size={13} className="group-hover:scale-110 transition-transform" /> Hỗ trợ: <span className="font-bold tracking-wide">1800 6750</span>
                </a>
                <div className="w-1 h-1 rounded-full bg-blue-400/50"></div>
                <a href="mailto:support@mtiensolution.vn" className="flex items-center gap-1.5 hover:text-white transition-colors group">
                  <Mail size={13} className="group-hover:scale-110 transition-transform" /> support@mtiensolution.vn
                </a>
              </div>
              ...
              <div className="flex items-center justify-center flex-1 gap-2.5 md:gap-3">
                <span className="flex bg-white/10 px-2.5 py-1 rounded-full text-[10px] items-center gap-1 backdrop-blur-md border border-white/20 font-bold tracking-widest text-white shadow-sm">
                  <Sparkles size={11} className="text-yellow-400 animate-pulse" /> GIẢI PHÁP CHUYỂN ĐỔI SỐ
                </span>
                <span className="font-medium truncate max-w-[200px] sm:max-w-max hidden md:block text-blue-50">Đồng hành cùng doanh nghiệp phát triển bền vững</span>
                <Link href="#" className="text-yellow-400 hover:text-yellow-300 flex items-center gap-1 ml-1 md:ml-3 font-semibold shrink-0 group transition-all" aria-label="Xem thêm ưu đãi">
                  Khám phá ngay
                            ...
                            return (
                              <Link key={subIdx} href={subLink.href} className="flex p-4 rounded-2xl hover:bg-gray-50 transition-all duration-300 group/item items-start gap-4" role="menuitem">
                                <div className="w-12 h-12 bg-white rounded-xl shadow-sm border border-gray-100 flex items-center justify-center shrink-0 group-hover/item:border-[#0066FF]/20 group-hover/item:bg-blue-50/50 transition-colors">
                                  <Icon className="w-6 h-6 text-[#0066FF] group-hover/item:scale-110 transition-transform duration-300" strokeWidth={1.5} />
                                </div>
                                <div>
                        ...
                        {link.subLinks.map((subLink, subIdx) => {
                          ...
                          return (
                            <Link key={subIdx} href={subLink.href} onClick={closeMenu} className="flex items-start gap-3 group">
                              {SubIcon && <div className="mt-0.5 w-6 h-6 rounded-md bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-blue-50 group-hover:text-[#0066FF] transition-colors"><SubIcon size={12} /></div>}
                              <div>
